...
if not input_path.is_file():
    ...
    try:
        ## This does not actually set the file mode in most instances,
        ## version-validator (which calls this) will have already created the
        ## output file with `mktemp`. There is no security benefit to locking
        ## down the file's permissions tigher than 644.
